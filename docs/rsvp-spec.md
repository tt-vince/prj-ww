...
| `components/envelope-reveal.tsx` | Client scroll-driven envelope→letter reveal (pinned; see status note). Ivory CSS envelope, landscape, **four flaps meeting at the centre** (no seal, sharp corners): the **top flap rotates open** (`rotateX`; z-index swapped in JS so it stays visible behind the letter); the front flaps (`.env-front` bottom + `.env-face-*` sides, z-index 12) tuck the letter's base. Letter = page content, base-anchored in `.letter-clip` (overflow-hidden; `.env-content` caps at `82dvh`). Eased `--p` via rAF scroll listener; `.env-cue`; `prefers-reduced-motion`. CSS in `globals.css` under "Envelope reveal". |
