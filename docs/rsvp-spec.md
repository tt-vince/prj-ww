...
| `components/envelope-reveal.tsx` | Client scroll-driven envelope→letter reveal (pinned, envelope always visible; see status note). CSS envelope = back panel + big **sealed top flap** (down-triangle, wax seal at tip) that tilts back + fades on open, cross-fading to a pale **open flap** (up-triangle standing behind the letter) + a **bottom chevron pocket** (`.env-front`, z-index 5, `pointer-events:none`) that tucks only the letter's base so form content is never covered. The letter IS the content, base-anchored in `.letter-clip` (overflow-hidden to the mouth; `.env-content` caps at `82dvh` with a styled scrollbar). Motion: rAF scroll listener writes an **eased** `--p`; sizing in `dvh`; "Scroll to open" cue (`.env-cue`); honors `prefers-reduced-motion`. CSS in `globals.css` under "Envelope reveal". |
