...
| `components/rsvp-form.tsx` | Client RSVP form, styled with shadcn/ui (`Button`/`Input`/`Label`/`Textarea`/`Separator`) + `has-[:checked]` radio-cards; "wisteria & fig". Adults/kids are −/+ steppers; a dietary-restrictions fieldset (preset checkboxes + **Other** → text) posts `dietary`/`dietaryOther`. Dietary uses **native `<input type=checkbox>`** (accent-coloured), matching the native attendance radios — not shadcn/base-ui `Checkbox`, whose controlled toggle scrolled the page inside the envelope reveal. |
