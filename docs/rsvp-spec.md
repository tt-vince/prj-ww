...
| `lib/dietary.ts` | Dietary presets (keys + labels) shared by the form, `rsvpResponseSchema`, the dashboard card, and the CSV export, plus `dietaryList()` for display. |
...
| `components/letter/rsvp-form.tsx` (+ `rsvp-form/` subcomponents — section shell, choice rows, dietary chips, companion cards, stepper, submit area) | Client RSVP form, styled with shadcn/ui (`Button`/`Input`/`Label`/`Textarea`/`Separator`) + `has-[:checked]` radio-cards; "wisteria & fig". Adults/kids are −/+ steppers — the whole **“Who is coming?”** section is hidden when `maxGuests === 1` (nothing to count: the invitee is adult 1 and no one else fits), and `adults`/`kids` post as hidden inputs (`1`/`0`) so the action reads a one-seat reply like any other; the dietary hint drops its “anyone you bring below” clause in that case. A dietary-restrictions fieldset (preset checkboxes + **Other** → text) posts `dietary`/`dietaryOther`. Dietary uses **native `<input type=checkbox>`** (accent-coloured), matching the native attendance radios — not shadcn/base-ui `Checkbox`, whose controlled toggle scrolled the page inside the envelope reveal. |
...
| `components/letter/rsvp.tsx` | Server component: the **RSVP** section, rendered OUTSIDE the envelope reveal (normal flow, below the letter — the reveal's percentage-based rise transform would jump the letter on the form's conditional-field height changes). A **solid deep forest green (`#2C3F25`, matching Our Story)** sits behind a single white shadcn `Card` holding the form. Awaits the forwarded `searchParams` to read `?id=<token>` inside its own `<Suspense>` (dynamic body; striped shell + card frame prerender). Three states: `RsvpForm` (pending), a thank-you (already answered), or a “reply by your personal link” note (no/unknown token). |
...
| `app/(protected)/dashboard/guests-board.tsx` (+ `board/` subcomponents — status column, guest card, toolbar/filters, mobile tabs, palette tokens, head-count helpers) | Client kanban board (imported design): 3 drag-and-drop status columns on `md+` (HTML5 dnd, `useOptimistic` status flip → `moveGuestStatus`), status tab pills + card list on phones; search + label-filter dropdown; per-column “Show more” (20 at a time). Column tints/inks hardcoded from the design (like the floral art). |
| `app/(protected)/dashboard/export-guests-button.tsx` | Client CSV export of the full guest list (includes a **Dietary** column). |
...
> **Folder convention (2026-07-27 restructure):** guest-letter components live under `components/letter/` (incl. `wedding-letter.tsx`, `rsvp-form.tsx`, and the retired `envelope-reveal.tsx` / in-band `vinyl-player.tsx`); dashboard-only components under `components/dashboard/` (`account-menu.tsx`, `florals.tsx`, `sns-icon.tsx`); components shared by both surfaces stay at the `components/` root (`countdown.tsx`, `providers.tsx`); shadcn primitives in `components/ui/`. Shared letter voice primitives: `components/letter/{section-heading.tsx,letter-button.ts,letter-type.ts}` (script h2 + sans kicker; the one letter button; tracked micro-caps field label). The unused `components/reveal.tsx` helper was deleted.
...
- **Guest fields:** name, `max_guests` allotment, optional email/phone, optional `admin_note`, plus
  ...
  `email`/`phone` (overwrites admin contact only when supplied) and **dietary restrictions**
  (`lib/dietary.ts` preset checkboxes + free-text Other). Reply stored as `status` + `adults` + `kids` +
