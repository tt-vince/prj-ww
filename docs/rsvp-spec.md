...
> **Status:** Admin auth + dashboard **built**. **Guest management built** — invitees, per-person invite tokens, editable labels, and admin CRUD on a **single-page “Manage RSVP”** dashboard at `/dashboard` (**kanban board** — see below — with search + label filter and CSV export; sidebar-less). Visuals follow the imported **hi-fi** Claude Design `Wedding RSVP Dashboard.dc.html` (“wisteria & fig”; fonts **DM Sans** / **Gilda Display** / **Pinyon Script**; gradient bg; mobile/iPad responsive; **decorative floral layer** — page-corner sprays, a name sprig, account-chip sprigs, and guest-card corner frames from the design, in `components/dashboard-florals.tsx`). The guest list follows the imported **kanban** designs (`Wedding RSVP - Kanban{,- Tablet,- Mobile}.dc.html`): desktop/tablet show three drag-and-drop status columns (Awaiting reply / Attending / Declined — warm tinted, dashed borders, dragging a card calls `moveGuestStatus`); phones swap the board for three status tab pills over the same card list (no drag — status changes via the edit dialog); on mobile each card carries its own small corner vine, cycling the corner (tl→tr→br→bl) per item via `CornerVine`. Each column carries its own **vine layer** on the outline (Awaiting = bottom-left sprig, Attending = full four-corner garland, Declined = top-right sprig — `ColumnVine{BottomLeft,Full,TopRight}` in `components/dashboard-florals.tsx`); the old board-wrapping `CardSpray*` frames are no longer used here. Column headers show the **guest count**; only **Attending** also shows a **seats** figure (total attending party size) — seats are meaningless for Awaiting/Declined. Each card shows a labeled **Replied** date (when answered) and groups guest + admin notes under one **Notes** section (color-coded Guest/Admin sub-labels). The former stat-card row and paginated table were replaced by the board; long columns paginate with a per-column “Show more” (20 at a time). The guest-facing RSVP form (the page a `?id=<token>` link opens) is **built** at `/` (unstyled for now): reads the invitee by token, shows a personalized greeting, hides the form when no/unknown token, and shows a confirmation when already answered. This is the source of truth for the RSVP feature.
